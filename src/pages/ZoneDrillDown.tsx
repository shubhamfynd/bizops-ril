...
const ZoneDrillDown: React.FC = () => {
  ...
  const getStateData = (): StateData[] => {
    ...
    return states.map((state, index) => {
      ...
      return {
        state,
        percentage: Math.round(statePercentage * 10) / 10,
        trend: Math.round((currentZoneData.trend + (Math.random() - 0.5) * 3) * 10) / 10,
            ...
            {stateData.map((data, index) => (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="text-gray-900 font-medium text-base">{data.state}</h3>
