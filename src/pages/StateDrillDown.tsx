...
import { ChevronLeft, TrendingUp, TrendingDown, Filter, ChevronRight, Home } from "lucide-react";
...
const StateDrillDown: React.FC = () => {
  ...
  const getCityData = (): CityData[] => {
    ...
    return cities.map((city, index) => {
      ...
      return {
        city,
        percentage: Math.round(cityPercentage * 10) / 10,
        trend: Math.round((currentZoneData.trend + (Math.random() - 0.5) * 4) * 10) / 10,
        direction: currentZoneData.direction,
        target: kpiType === 'ats' ? '₹2,500' : 
                kpiType === 'ipcm' ? '2.5' : 
                kpiType === 'bills-vs-footfall' ? '9,750' :
                '₹10,50,000',
        actual: kpiType === 'ats' ? `₹${(cityPercentage * 30).toFixed(0)}` : 
                kpiType === 'ipcm' ? cityPercentage / 40 : 
                kpiType === 'bills-vs-footfall' ? Math.round(cityPercentage * 125).toLocaleString() :
                `₹${(cityPercentage * 10000).toLocaleString()}`,
            ...
            {cityData.map((data, index) => (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="text-gray-900 font-medium text-base">{data.city}</h3>
